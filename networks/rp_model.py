...
def make_r_model(
    ...
    mint = ModalityInteraction(d_model, d_ff, dropout, use_effect=False)
...
class ReasonModel(nn.Module):
    """
    Edited from a standard Encoder-Decoder architecture.
    """

    def __init__(self, mid, encoder_qsvo, encoder_obj, fuse_node,
                 src_embed, tgt_embed, mid_embed, mid_generator, obj_generator):
        super(ReasonModel, self).__init__()
        self.mid = mid
    ...
    def forward(self, src, src_mask=None, qsvo_supervision=None, obj_supervision=None,
                replace_indices=None, object_indices=None, gt_qsvo=False):
        ...
        if self.mid == 'qsvo+obj':
            ...
            obj_output = self.obj_generator(obj_hidden, src_mask[0])
            return obj_hidden, qsvo_output, obj_output
...
class PlanModel(nn.Module):
    ...
    def __init__(self, mid, encoder_action, fuse_node, src_embed, tgt_embed, mid_embed, generator):
    ...
    def forward(self, src, src_mask=None, qsvo_supervision=None, obj_supervision=None,
        ...
        if self.mid == 'qsvo' or self.mid == 'qsvo+obj':
            ...
            qsvo_x = torch.cat((soc_x[:, 0:1], qsvo_x[0].unsqueeze(dim=1), qsvo_x[1].unsqueeze(dim=1),
                                qsvo_x[2].unsqueeze(dim=1), qsvo_x[3].unsqueeze(dim=1)), dim=1)
        if self.mid =='qsvo':
            qsvo_mask = (src_mask[0], torch.ones([qsvo_x.shape[0], qsvo_x.shape[1]]).to(qsvo_x.device),
                         src_mask[2], src_mask[3])
            hidden = self.encoder_action(phy_x, qsvo_x, qry_x, eff_x, qsvo_mask)
            return hidden
        ...
        if self.mid == 'obj' or self.mid == 'qsvo+obj':
            ...
            obj_x = torch.cat((qry_x[:, 0:1], target_obj_token.unsqueeze(dim=1)), dim=1)
        if self.mid == 'obj':
            obj_mask = (src_mask[0], src_mask[1],
                        torch.ones([obj_x.shape[0], obj_x.shape[1]]).to(obj_x.device), src_mask[3])
            hidden = self.encoder_action(phy_x, soc_x, obj_x, eff_x, obj_mask)
            return hidden

        if self.mid == 'qsvo+obj':
            qsvo_obj_mask = (src_mask[0], torch.ones([qsvo_x.shape[0], qsvo_x.shape[1]]).to(qsvo_x.device),
                             torch.ones([obj_x.shape[0], obj_x.shape[1]]).to(obj_x.device), src_mask[3])
            hidden = self.encoder_action(phy_x, qsvo_x, obj_x, eff_x, qsvo_obj_mask)
            return hidden
