...
def e2e_eval(args):
    ...
    e2e_model = make_e2e_model(args.mid, src_vocab, tgt_vocab, mid_vocabs, device,
                               d_model=args.d_model, N_qsvo=args.N_qsvo, N_obj=args.N_obj)
    e2e_save_name = '_'.join([args.experiment_name, 'e2e', args.mid,
                              'depth%d%d%d' % (args.N_qsvo, args.N_obj, args.N_action),
                              'width%d' % args.d_model, 'seed%d' % args.e2e_seed, 'epoch%d' % args.e2e_epoch_id])
    e2e_state_dict = torch.load(os.path.join(args.save_path, e2e_save_name))
    e2e_model.load_state_dict(e2e_state_dict)
    e2e_model = e2e_model.to(device)

    loss_compute = get_loss_compute(e2e_model.generator)
    e2e_model.eval()
    with torch.no_grad():
        episode_counter = 0
        action_counter = 0
        ...
        for file_idx, file in enumerate(test_files):
            ...
            for sol_idx, sol in enumerate(solutions):
                ...
                hidden, qsvo_output, obj_output = e2e_model.forward(
                    batch.src,
                    batch.src_mask,
                    batch.qsvo,
